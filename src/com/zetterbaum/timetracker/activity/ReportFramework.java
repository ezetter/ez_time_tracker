...
import com.zetterbaum.timetracker.report.ReportOutput;
import com.zetterbaum.timetracker.report.SDDataExporter;
...
public class ReportFramework implements Serializable {
	...
	ReportFramework(Activity activity, ReportInterface report) {
        ...
        showSeparator = sharedPrefs.getBoolean("show_report_separator", false);
    }
	...
	LinearLayout buildViews() {
		...
		startDateTV.setText(Constants.FROM_DATE.getValue() + ": " + DateFormat.format("E, MMMM dd, yyyy", startDateRange),
		...
		endDateTV.setText(Constants.TO_DATE.getValue() + ": " + DateFormat.format("E, MMMM dd, yyyy", endDateRange),
				TextView.BufferType.SPANNABLE);
    ...
    public void addSeparator(LinearLayout layout) {
        if (showSeparator) {
            View separator = new View(activity);
            separator.setMinimumHeight(3);
            separator.setBackgroundDrawable(activity.getResources().getDrawable(R.drawable.report_separator));
